...
export default async function InvoicesPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const { data: allInvoices } = await supabase
    .from('invoices')
    .select('*, customer:customers(name), project:projects(name), invoice_tax_invoices(amount, invoice_payments(amount))')
              ...
              {filtered.map((inv: any) => (
                ...
                >
                  ...
                  <div className="text-right shrink-0 ml-4">
                    ...
                    {Array.isArray(inv.invoice_tax_invoices) && inv.invoice_tax_invoices.length > 0 && (() => {
                      const paid = (inv.invoice_tax_invoices as any[]).reduce((s, ti) =>
                        s + (ti.invoice_payments as any[]).reduce((ps: number, p: any) => ps + (p.amount || 0), 0), 0)
